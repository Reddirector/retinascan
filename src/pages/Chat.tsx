...
export default function Chat() {
  ...
  const handleFile = useCallback(
    (file: File) => {
      ...
      // Backend lookup — may include a live AI report call, so it is kicked
      // off in parallel with the animation and awaited at the reveal.
      type ScreenResponse =
        | { error: string }
        | { matched: false }
        | MatchedResult;
      ...
      timersRef.current.push(
        setTimeout(async () => {
          ...
          if ("error" in data) {
            pushEntry({ kind: "assistant", text: `⚠ ${data.error}` });
            return;
          }
          if (!data.matched) {
            pushEntry({
              kind: "assistant",
              text: "No demo case configured for this file. Try eyescan1 / eyescan2 / eyescan3.",
            });
            return;
          }
